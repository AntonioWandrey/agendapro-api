...
export default class UserController {
    ...
    public static async newUser(req: Request, res: Response) {
        const {
            nome, sobreNome, cpf, rg, departamento, email, cargoId,
            numeroDeTelefone, senha, confirmacaoDeSenha, empresaId,
        } = req.body;
        ...
        try {
            const token = await UserService.newUser({
                nome, sobreNome, cpf, rg, departamento, email,
    ...
    public static async loginUser(req: Request, res: Response) {
        ...
        try {
            ...
            return res.status(200).json({ message: "Login bem-sucedido.", token: result, });
        } catch (err: any) {
